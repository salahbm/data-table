...
import { WITH_ROW_ACTION_COLUMNS } from './with-row-action-columns'

interface WithRowActionTableProps {
  initialData: {
    data: Product[]
    meta: {
      page: number
...
export const WithRowActionTable = ({ initialData }: WithRowActionTableProps) => {
  ...
  const { table } = useDataTable<Product>({
    data: data,
    columns: WITH_ROW_ACTION_COLUMNS,
