...
import { WITH_DYNAMIC_ROW_COLUMNS } from './with-dynamic-row-columns'

interface WithDynamicRowTableProps {
  initialData: {
    data: Product[]
    meta: {
      page: number
...
export const WithDynamicRowTable = ({
  ...
  const { table } = useDataTable<Product>({
    data: data,
    columns: WITH_DYNAMIC_ROW_COLUMNS,
